...
export default function highlightContainer(element) {
    ...
    CSimage.layers.forEach((layer, index) => {
        ...
        cornerstone.loadAndCacheImage(CSimage.projection + ':' + defineVariables().fileFormats[layer.format] + layer.baseURL + '/' + CSimage.projection + '/' + layer.stack[CSimage.currentTimeIndex].imageIds[CSimage.currentImageIdIndex]).then(image => {
