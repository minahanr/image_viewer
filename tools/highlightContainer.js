...
export default function highlightContainer(element) {
    ...
    CSimage.layers.forEach((layer, index) => {
        ...
        cornerstone.loadAndCacheImage(defineVariables().fileFormats[layer.format] + CSimage.baseURL + '/' + CSimage.projection + '/' + layer.stack[CSimage.currentTimeIndex].imageIds[CSimage.currentImageIdIndex]).then(image => {
