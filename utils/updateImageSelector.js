    ...
    CSimage.layers.forEach(layer => {
        if (CSimage.currentImageIdIndex < layer.startingSpaceIndex + layer.stack[0].imageIds.length && CSimage.currentImageIdIndex >= layer.startingSpaceIndex &&
            CSimage.currentTimeIndex < layer.startingSpaceIndex + layer.stack[0].imageIds.length && CSimage.currentTimeIndex >= layer.startingTimeIndex) {
                ...
                console.log(defineVariables().fileFormats[layer.format] + layer.baseURL + '/' + CSimage.projection + '/' + layer.stack[CSimage.currentTimeIndex].imageIds[CSimage.currentImageIdIndex]);
                promises.push(cornerstone.loadAndCacheImage(defineVariables().fileFormats[layer.format] + layer.baseURL + '/' + CSimage.projection + '/' + layer.stack[CSimage.currentTimeIndex].imageIds[CSimage.currentImageIdIndex]));
        } 
    });
