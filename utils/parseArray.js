export default function parseArray(imageIndex) {
    ...
    for (let i = 0; i < numFrames; i++) {
        urlsOverTime.push([]);
        for(let j = 0; j < imgsPerFrame; j++) {
            urlsOverTime[i].push(baseURL + '/' + '0'.repeat(Math.floor(Math.log10(numFrames)) - Math.floor(Math.log10(Math.max(i + startingTimeIndex, 1)))) + (i + startingTimeIndex) + '-' + '0'.repeat(Math.floor(Math.log10(imgsPerFrame)) - Math.floor(Math.log10(Math.max(j + startingSpaceIndex, 1)))) + (j + startingSpaceIndex) + '.' + format);
        }
    }
