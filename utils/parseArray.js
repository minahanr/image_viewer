export default function parseArray(imageIndex, projection) {
    ...
    for (let i = 0; i < numFrames; i++) {
        ...
        for(let j = 0; j < imgsPerFrame; j++) {
            ...
            urlsOverTime[i].push('0'.repeat(Math.floor(Math.log10(numFrames)) - Math.floor(Math.log10(Math.max((i + 1), 1)))) + (i  + 1) + '-' + '0'.repeat(Math.floor(Math.log10(imgsPerFrame)) - Math.floor(Math.log10(Math.max(j + 1, 1)))) + (j + 1) + '.' + format + '?raw=true');
        }
    }
